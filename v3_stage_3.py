...
def load_and_validate_communities(filepath: str, cde_lookup: dict) -> List[ParentCommunity]:
    ...
    try:
        with open(filepath, 'r', encoding='utf-8') as f:
            ...
            validated_data: List[ParentCommunity] = [ParentCommunity.model_validate(item) for item in data]
        ...
        # User approval step does not need to be run for a single test
        return validated_data
    except (ValidationError, json.JSONDecodeError, TypeError, IndexError, KeyError) as e:
        logging.error(f"Failed to load or validate community file: {e}"); return []
...
def generate_content_via_rest(prompt_text: str, cache_name: str, api_key: str) -> dict:
    ...
    try:
        group_id_from_prompt = json.loads(prompt_text).get("group_id_for_request", "unknown")
        raw_response_path = os.path.join(RAW_DIR, f"{group_id_from_prompt}_response.json")
        with open(raw_response_path, 'w', encoding='utf-8') as f:
            json.dump(resp.json(), f, indent=2)
    except Exception: # If response is not JSON, save raw text
        with open(raw_response_path, 'w', encoding='utf-8') as f:
            f.write(resp.text)
...
def process_group(
    ...
    try:
        ...
        # Add a field for logging inside the REST function
        prompt_payload = {
            "group_id_for_request": group_id,
            "cde_group_for_review": cde_group_data,
            "community_context": community_context_text
        }
...
def main(api_key: str):
    ...
    if not cde_lookup:
        logging.fatal("Could not load CDE catalog. Exiting."); return
    ...
    for community in tqdm(community_definitions, desc="Processing Communities"):
        ...
        if not groups_to_process:
            logging.info(f"All groups in {community_id} already processed. Skipping."); continue
        
        logging.info(f"Processing {len(groups_to_process)} groups for community {community_id}.")
        try:
            cache_name = create_cache_via_rest(api_key, SYSTEM_PROMPT, community_context_text)
            if not cache_name:
                logging.error(f"Failed to create cache for {community_id}. Skipping."); continue

            with concurrent.futures.ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
                future_to_group = {
                    executor.submit(process_group, group, cde_lookup, cache_name, api_key): group
                ...
                for future in tqdm(concurrent.futures.as_completed(future_to_group), total=len(groups_to_process), desc=f"Groups in {community_id}"):
                    ...
                    try:
                        ...
                        status, usage = result.get("status", "unknown_error"), result.get("usage")
                        ...
                        if total_cost >= COST_LIMIT_USD:
                            logging.fatal(f"COST LIMIT REACHED (${total_cost:.2f}). Halting."); return
                    except Exception as exc:
                        manifest[group_id] = "future_failed"
                        log_error(group_id, exc, {"note": "Error retrieving result from future."})
            save_manifest(MANIFEST_FILE, manifest)
        finally:
            if cache_name:
                delete_cache(cache_name, api_key)
    
    with open(os.path.join(OUTPUT_DIR, "adjudication_results.json"), 'w', encoding='utf-8') as f:
        json.dump(all_results, f, indent=2)
    logging.info(f"--- Stage 3 Adjudication Complete. Total Estimated Cost: ${total_cost:.4f} USD ---")

# --- 7. Corrected Test Function ---
def run_single_test(api_key: str):
    """
    ...
    cde_lookup = load_cde_catalog(PROCESSED_CATALOG_PATH)
    if not cde_lookup:
        logging.error("Failed to load CDE catalog for test."); return

    # In a test, we don't need user approval, so we pass a dummy lookup dict
    # and disable the input prompt by modifying the function temporarily if needed.
    # For now, we will assume it's okay to have the prompt.
    communities = load_and_validate_communities(INPUT_COMMUNITIES_PATH, cde_lookup)
    if not communities:
        logging.error("Failed to load communities for test."); return

    # --- 3. FIND THE TARGET GROUP AND ITS PARENT CONTEXT ---
    target_group, community_context = None, ""
    for community in communities:
        for group in community.sub_groups:
            if group.group_id == TEST_GROUP_ID:
                # FIX: Use model_dump() for Pydantic V2 compatibility
                target_group = group.model_dump()
                member_titles = [cde_lookup.get(str(id), {}).get('title', '') for id in community.member_cde_ids]
                community_context = "\n- ".join(filter(None, member_titles))
                break
        if target_group: break
    
    if not target_group:
        logging.error(f"Could not find group with ID '{TEST_GROUP_ID}'"); return

    cache_name = None
    try:
        logging.info("Creating temporary cache for single test...")
        cache_name = create_cache_via_rest(api_key, SYSTEM_PROMPT, community_context)
        if not cache_name:
            logging.error("Failed to create cache for test run."); return

        logging.info("Calling process_group directly for the test group...")
        # --- FIX: The `community_context_text` argument is now correctly passed ---
...
# --- 8. Main Execution Guard (Corrected for .env and Test Run) ---
if __name__ == "__main__":
    ...
    # 2. **FIX**: Explicitly load the .env file into the environment
    try:
        from dotenv import load_dotenv
        load_dotenv()
        logging.info("Successfully loaded environment variables from .env file.")
    except ImportError:
        logging.warning("`python-dotenv` library not found. Could not load .env file.")
        logging.warning("Please run `pip install python-dotenv` to fix.")

    # 3. Configure API Key
    api_key = os.getenv("GOOGLE_API_KEY")
    if not api_key:
        logging.fatal("FATAL: GOOGLE_API_KEY environment variable not found.")
        raise ValueError("GOOGLE_API_KEY must be set in your environment or .env file.")
    
    logging.info("Successfully loaded GOOGLE_API_KEY.")
    
    # 4. Run the single test function as requested
    run_single_test(api_key=api_key)